...
{
	if (l.isEmpty()) // if list is empty
	{
		return *this;
	}
	if (this->head == NULL) // if this is not empty
	{
		this->clear();
	}
	Link *p1, *p2; // pointer
	this->head = new Link(l.head->value, NULL); // creat head
	p1 = l.head; // point to head
	p2 = this->head; // point to head
	while (p1->next != NULL) // as long not end of list
	{	
		p2->next = new Link(p1->next->value, NULL); // copy
		p2 = p2->next; // next 
		p1 = p1->next; // next 
	}
	return *this; // return 
